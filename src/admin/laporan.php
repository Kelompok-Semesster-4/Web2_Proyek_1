...
foreach ($statusCountsRaw as $row)
    $statusCounts[(int) $row['id']] = ['nama' => (string) $row['nama_status'], 'jumlah' => (int) $row['jumlah']];
...
foreach ($topRuangan as &$room)
    $room['total_jam'] = normalize_time_text((string) ($room['total_jam'] ?? '00:00:00'));
unset($room);
...
$monthlyVisitMap = [];
foreach ($monthlyVisitsRaw as $row)
    $monthlyVisitMap[(int) $row['month_num']] = $row;
...
foreach ($dailyActivity as $d) {
    $dailyLabels[] = date('d M', strtotime((string) $d['tanggal']));
...
<div class="admin-container" style="max-width:100%;">
    <div class="kelola-header mb-4">
        ...
        <p class="text-muted mb-0">Seluruh chart bulanan, statistik penggunaan, dan tabel transaksi detail ada di
            halaman ini.</p>
    </div>
    ...
    <div class="card shadow border-0 mb-4">
        ...
        <div class="card-body">
            <form class="row g-3 align-items-end" method="GET">
                <div class="col-md-2"><label class="form-label">Bulan</label><select name="month"
                        class="form-select"><?php for ($m = 1; $m <= 12; $m++): ?>
                            <option value="<?= $m ?>" <?= $m === $month ? 'selected' : '' ?>>
                                <?= date('F', mktime(0, 0, 0, $m, 1)) ?></option><?php endfor; ?>
                    </select></div>
                <div class="col-md-2"><label class="form-label">Tahun</label><select name="year"
                        class="form-select"><?php $yNow = (int) date('Y');
                        for ($y = $yNow - 3; $y <= $yNow + 1; $y++): ?>
                            <option value="<?= $y ?>" <?= $y === $year ? 'selected' : '' ?>><?= $y ?></option><?php endfor; ?>
                    </select></div>
                <div class="col-md-4"><label class="form-label">Ruangan</label><select name="ruangan_id"
                        class="form-select">
                        <option value="0">Semua Ruangan</option><?php foreach ($ruanganList as $r): ?>
                            <option value="<?= (int) $r['id'] ?>" <?= ((int) $r['id'] === $ruanganId) ? 'selected' : '' ?>>
                                <?= e(($r['gedung'] ?? '-') . ' - ' . $r['nama_ruangan']) ?></option><?php endforeach; ?>
                    </select></div>
                <div class="col-md-2"><label class="form-label">Status</label><select name="status_id"
                        class="form-select">
                        <option value="0">Semua Status</option><?php foreach ($statusList as $s): ?>
                            <option value="<?= (int) $s['id'] ?>" <?= ((int) $s['id'] === $statusId) ? 'selected' : '' ?>>
                                <?= e($s['nama_status']) ?></option><?php endforeach; ?>
                    </select></div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-3">
            <div class="stat-card-slim bg-primary">
                <div class="stat-card-header"><span class="stat-value"><?= $totalRequests ?></span></div>
                <div class="stat-card-label">Total Request</div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="stat-card-slim bg-success">
                <div class="stat-card-header"><span class="stat-value"><?= number_format($approvalRate, 1) ?>%</span>
                </div>
        ...
        <div class="col-md-3">
            <div class="stat-card-slim bg-danger">
                <div class="stat-card-header"><span class="stat-value"><?= number_format($rejectionRate, 1) ?>%</span>
                </div>
                <div class="stat-card-label">Rejection Rate</div>
            </div>
        </div>
        <div class="col-md-3">
            <div class="stat-card-slim bg-info">
                <div class="stat-card-header"><span class="stat-value"
                        style="font-size:20px;"><?= e($avgDurasi) ?></span></div>
                <div class="stat-card-label">Rata-rata Durasi</div>
            </div>
        </div>
    </div>
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm">
                <div class="card-header bg-light">
                    <h6 class="mb-0">Daily Activity (<?= e(date('F Y', strtotime($startDate))) ?>)</h6>
                </div>
    ...
    <div class="row g-3 mb-4">
        <div class="col-lg-4">
            <div class="card shadow-sm h-100">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between mb-2"><span>Total Jam
                            Digunakan</span><strong><?= e($totalJam) ?></strong></div>
                    <div class="d-flex justify-content-between mb-2"><span>Total Jam
                            (desimal)</span><strong><?= number_format($usedHours, 2) ?> jam</strong></div>
                    <div class="d-flex justify-content-between mb-2"><span>Kapasitas
                            Jam</span><strong><?= number_format($capacityHours, 0) ?> jam</strong></div>
                    <div class="d-flex justify-content-between mb-3"><span>Utilization
                            Rate</span><strong><?= number_format($utilizationRate, 1) ?>%</strong></div>
                    ...
                    <table class="table table-hover mb-0 align-middle">
                        ...
                        <tbody><?php if (!$topRuangan): ?>
                            ...
                            foreach ($topRuangan as $i => $room): ?>
                                    <tr>
                                        <td><?= $i + 1 ?></td>
                                        <td><?= e($room['nama_ruangan']) ?><br><small
                                                class="text-muted"><?= e($room['gedung'] ?? '-') ?></small></td>
                                        <td class="text-center"><span
                                                class="badge bg-info"><?= (int) $room['jumlah_booking'] ?></span></td>
                                        <td class="text-center"><span
                                                class="badge bg-success"><?= e($room['total_jam']) ?></span></td>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-light d-flex justify-content-between align-items-center">
            <h6 class="mb-0">Detail Transaction Table</h6><a class="btn btn-sm btn-success"
                href="?year=<?= $year ?>&month=<?= $month ?>&ruangan_id=<?= $ruanganId ?>&status_id=<?= $statusId ?>&export=csv">Export
                CSV</a>
        </div>
        <div class="table-responsive">
            <table class="table table-hover mb-0 align-middle">
                ...
                <tbody><?php if (!$detail): ?>
                    ...
                    <?php else:
                    $statusBadgeMap = ['Menunggu' => 'warning', 'Disetujui' => 'success', 'Ditolak' => 'danger', 'Selesai' => 'info', 'Dibatalkan' => 'secondary'];
                    foreach ($detail as $idx => $d):
                        $badgeClass = $statusBadgeMap[$d['nama_status']] ?? 'secondary'; ?>
                            <tr>
                                <td><?= $idx + 1 ?></td>
                                <td><?= e(date('d M Y', strtotime((string) $d['tanggal']))) ?></td>
                                <td><?= e(substr((string) $d['jam_mulai'], 0, 5)) ?> -
                                    <?= e(substr((string) $d['jam_selesai'], 0, 5)) ?></td>
                                <td><?= e($d['nama_ruangan']) ?><br><small
                                        class="text-muted"><?= e($d['gedung'] ?? '-') ?></small></td>
                                <td><?= e($d['nama_peminjam']) ?></td>
                                <td><?= !empty($d['prodi']) ? e((string) $d['prodi']) : '-' ?></td>
                                <td><?= e($d['nama_kegiatan']) ?></td>
                                <td class="text-center"><span
                                        class="badge bg-info"><?= e((string) ($d['jumlah_peserta'] ?? '-')) ?></span></td>
                                <td class="text-center"><span
                                        class="badge bg-<?= $badgeClass ?>"><?= e($d['nama_status']) ?></span></td>
                                <td><?= !empty($d['catatan_admin']) ? e((string) $d['catatan_admin']) : '-' ?></td>
                            </tr><?php endforeach; endif; ?>
                </tbody>
            </table>
        </div>
