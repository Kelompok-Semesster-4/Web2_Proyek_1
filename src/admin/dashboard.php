...
<div class="admin-container" style="max-width:100%;">
    ...
    <div class="row g-3 mb-4">
        <div class="col-md-4"><div class="stat-card-slim bg-primary"><div class="stat-card-header"><span class="stat-value"><?= $bookingHariIni ?></span></div><div class="stat-card-label">Booking Hari Ini</div></div></div>
        <div class="col-md-4"><div class="stat-card-slim bg-success"><div class="stat-card-header"><span class="stat-value"><?= $ruanganTerpakai ?></span></div><div class="stat-card-label">Ruangan Sedang Dipakai</div></div></div>
        <div class="col-md-4"><div class="stat-card-slim bg-danger"><div class="stat-card-header"><span class="stat-value"><?= $pendingTotal ?></span></div><div class="stat-card-label">Pending Approval (Semua)</div></div></div>
    </div>

    <div class="row g-3 mb-4">
        <div class="col-lg-4">
            <div class="card shadow-sm top-equal-card"><div class="card-header bg-light"><h6 class="mb-0">Status Ruangan Saat Ini</h6></div><div class="card-body room-status-body"><canvas id="roomStatusChart"></canvas></div></div>
        </div>
        <div class="col-lg-8">
            <div class="card shadow-sm top-equal-card pending-equal-card">
                ...
                <div class="table-responsive dashboard-scroll-thin">
                    <table class="table table-hover mb-0 align-middle" id="pendingCompactTable">
                        <thead><tr><th>#</th><th>Mahasiswa</th><th>Ruangan</th><th>Waktu</th><th>Kegiatan</th><th>Aksi</th></tr></thead>
                        <tbody>
                        <?php if (!$pendingList): ?>
                            <tr><td colspan="6" class="text-center py-4 text-muted">Tidak ada antrean pending.</td></tr>
                        <?php else: foreach ($pendingList as $i => $item): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= e($item['nama_user']) ?><br><small class="text-muted"><?= e($item['prodi'] ?? '-') ?></small></td>
                                <td><?= e($item['nama_ruangan']) ?><br><small class="text-muted"><?= e($item['gedung'] ?? '-') ?></small></td>
                                <td><?= e(date('d M Y', strtotime((string) $item['tanggal']))) ?><br><small class="text-muted"><?= e(substr((string) $item['jam_mulai'], 0, 5)) ?> - <?= e(substr((string) $item['jam_selesai'], 0, 5)) ?></small></td>
    ...
    <div class="row g-3">
        <div class="col-lg-8">
            <div class="card shadow-sm"><div class="card-header bg-light"><h6 class="mb-0">Today's Schedule</h6></div>
                <div class="table-responsive dashboard-scroll-220">
                    <table class="table table-hover mb-0 align-middle" id="todayScheduleCompactTable">
                        <thead><tr><th>Jam</th><th>Ruangan</th><th>Peminjam</th><th>Kegiatan</th><th>Status</th></tr></thead>
                        <tbody>
                        <?php if (!$jadwalHariIni): ?>
                            <tr><td colspan="5" class="text-center py-4 text-muted">Belum ada jadwal hari ini.</td></tr>
                        <?php else: foreach ($jadwalHariIni as $jadwal): ?>
                            <tr>
                                <td><?= e(substr((string) $jadwal['jam_mulai'], 0, 5)) ?> - <?= e(substr((string) $jadwal['jam_selesai'], 0, 5)) ?></td>
                                <td><?= e($jadwal['nama_ruangan']) ?><br><small class="text-muted"><?= e($jadwal['gedung'] ?? '-') ?></small></td>
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm"><div class="card-header bg-light"><h6 class="mb-0">Status Hari Ini</h6></div><div class="card-body" style="height:220px;"><canvas id="todayStatusChart"></canvas></div></div>
        </div>
    </div>
</div>
